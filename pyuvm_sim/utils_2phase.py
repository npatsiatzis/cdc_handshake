...
class HandShake2PhaseBfm(metaclass=utility_classes.Singleton):
    def __init__(self):
        self.dut = cocotb.top
    ...
    async def driver_bfm(self):
        while True:
            await RisingEdge(self.dut.i_clk_A)
            try:
                (valid,data) = self.driver_queue.get_nowait()
                self.dut.i_valid_A.value = valid
    ...
    async def data_mon_bfm(self):
        while True:
            await FallingEdge(self.dut.o_ready_A)
            data = self.dut.o_data_A.value
            self.data_mon_queue.put_nowait(data)

    async def result_mon_bfm(self):
        while True:
            await RisingEdge(self.dut.o_valid_B)
